...
import {
  ...
  EmailCodeStoreEntry,
  EmailCodeStorePort,
} from '../ports/email-code-store.port';
...
export class AuthEmailCodeService {
  ...
  constructor(
    ...
    @Inject(EMAIL_CODE_STORE)
    private readonly emailCodeStore: EmailCodeStorePort,
  ) {}
  ...
  private async getValidStoredCode(
    normalizedEmail: string,
  ): Promise<EmailCodeStoreEntry | null> {
    const storedCode = await this.emailCodeStore.get(normalizedEmail);
    if (!storedCode) {
      return null;
    }

    if (storedCode.expiresAt < Date.now()) {
      await this.emailCodeStore.delete(normalizedEmail);
      return null;
    }
  ...
  async sendCode(normalizedEmail: string): Promise<void> {
    const existingCode = await this.getValidStoredCode(normalizedEmail);
    ...
    await this.emailCodeStore.set(normalizedEmail, {
  ...
  async resendCode(normalizedEmail: string): Promise<void> {
    const storedCode = await this.getValidStoredCode(normalizedEmail);
    if (!storedCode) {
    ...
    await this.emailCodeStore.set(normalizedEmail, storedCode);
  }

  /** @description 이메일 인증 코드를 검증한다. */
  async verifyCode(normalizedEmail: string, code: string): Promise<void> {
  ...
  async clearCode(normalizedEmail: string): Promise<void> {
    await this.emailCodeStore.delete(normalizedEmail);
  }
}
